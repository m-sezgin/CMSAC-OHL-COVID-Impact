...
ohl_treatment_prep <- ohl %>%
  ...
  mutate(gp_covid = sum(gp),
         ...
         covid_league = league) %>%
  ungroup() %>%
  select(player_id, gp_covid, championship, covid_league) %>%
...
ohl_21_22_ppg <- ohl_szn %>%
  ...
  mutate(ppg_21_22 = sum(pts)/sum(gp),
         gp_21_22 = sum(gp)) %>%
  select(player_id, ppg_21_22, gp_21_22)
...
ohl_filtered <- ohl_filtered %>%
  select(season, player_id, championship, gp_19_20, first_name, last_name, position, age, gp_19_20, pts_19_20, ppg_19_20, drafted, draft_year, round, overall_pick_num, age_continuous, pm_relative_19_20, pm_rank_19_20, pm_19_20, treatment, ppg_21_22, gp_covid, gp_21_22, covid_league)
```
