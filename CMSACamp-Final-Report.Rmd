...
ohl_21_22_ppg <- ohl_szn %>%
  ...
  mutate(ppg_21_22 = sum(pts)/sum(gp),
         pts_21_22 = sum(pts),
         gp_21_22 = sum(gp)) %>%
  dplyr::select(player_id, ppg_21_22, pts_21_22, gp_21_22)
...
ohl_filtered <- ohl_filtered %>%
  dplyr::select(season, player_id, championship, gp_19_20, first_name, last_name, position, age, gp_19_20, pts_19_20, ppg_19_20, drafted, draft_year, round, overall_pick_num, age_continuous, pm_relative_19_20, pm_rank_19_20, pm_19_20, treatment, ppg_21_22, gp_covid, gp_21_22, covid_league, pts_21_22)
```
...
ggplot(ohl_filtered, aes(x = gp_19_20, color = treatment)) +
  geom_density() +
  labs(title = "Players\nwho played during COVID season played more games in pre-COVID season", 
...
# yes it is, but the only significnt term is ppg*gp, which is just points...
```
...
ohl_mlr_final <- lm(ppg_21_22 ~ position + ppg_19_20 + treatment + pts_19_20 + gp_21_22 + age_continuous, data = ohl_filtered)
#summary(ohl_mlr_final)
```

#### Propensity Score Matching

The first match we tried was using the nearest neighbor method. This approach does not optimize criterion, so we switched to optimal pair matching. The absolute within-pair difference of each covariate was smaller in the optimal match, hence being better balanced. 
...
\(\widehat{\text{Player Performance}} = 0.904 + 0.0601*\text{Treatment} + 0.189*\text{Forward} + 0.318*\text{Player Quality} + 0.00548*\text{Games Played Post-COVID}\\- 0.0709*\text{Age} + 0.0102*\text{Points Pre-COVID} \)
...
* The model explains approximately 61% of the variation in player performance in the post-COVID season, which is a statistically significant amount at the \(\alpha = .001\) level \(F(6, 212) = 57.62, p<.001\).
...
Player performance in the post-COVID season is expected to increase by .0601 PPG on average for skaters who played during COVID, when holding constant position, player quality, games played in the post-COVID season, points scored in the pre-COVID season, and continuous age.
...
We are 95% confident that the change in player performance in the post-COVID for players who played during the COVID season is between -0.022 and 0.142 PPG, holding constant position, player quality, games played in the post-COVID season, points scored in the pre-COVID season, and continuous age.
