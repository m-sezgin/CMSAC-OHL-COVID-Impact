...
# updated Michele's midpoint code
...
ohl_filtered <- ohl_filtered %>%
  select(season, player_id, championship, gp_covid, first_name, last_name, position, age, gp_total, pts_total, ppg_total, drafted, draft_year, round, overall_pick_num, age_continuous, pm_relative, pm_rank, pm, treatment)

ohl_pivoted <- ohl_filtered %>%
  pivot_wider(names_from = season, values_from = ppg_total)
...
```{r}
ggplot(ohl_treatment_21_22, aes(x = gp_covid, y = ppg_total)) +
  geom_point(alpha = .5) +
  theme_bw() +
  labs(title = "Positive? Relationship between GP during COVID season and PPG in 2021-2022 season")
```

```{r}
ggplot(ohl_treatment_21_22, aes(color = championship, x = ppg_total)) +
  geom_density() +
  theme_bw() +
  labs(title = "OHL skaters who played in a world championship during COVID perform better in 2021-2022 season",
       x = "PPG") 
```
...
filtered <- ohl_filtered %>% 
  mutate(num_treatment = ifelse(treatment == TRUE, 1, 0))
...
```{r}
library(bartCause)
bart_mod <- bartc(ppg_total, treatment, championship + position + age_continuous + gp_total + gp_covid + drafted + pm_relative, n.samples = 100L, n.burn = 15L, n.chains = 2L, data = ohl_filtered, method.trt = "bart")
summary(bart_mod)
```
...
1. Right now we are predicting ppg in both 2019-10 and 2021-22. Should we just be predicting ppg in 2021-2022, using ppg in 2019-2021 as a predictor variable?
